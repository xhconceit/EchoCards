...
macOS 与 Android 在系统服务、窗口模型、文件路径、音频输入和语音 API 上存在差异。Android 的 `Context`、Room、`TextToSpeech`、权限和生命周期代码不能直接复用到 macOS。
...
| 文字转语音 | Android `TextToSpeech` | `AVSpeechSynthesizer` |
| 跟读识别 | Android Vosk 适配器 | 单独验证的 Vosk macOS 适配器或 macOS 语音 API |
...
- macOS 端可以直接使用 Apple 的语音、音频和文件系统 API。
...
4. 接入 macOS 文字转语音。
5. 单独验证 macOS 跟读识别；在识别方案稳定前，桌面端提供手动学习模式。
6. 增加 macOS 打包、签名、公证和 Apple Silicon 构建流程。

## 8. 验收与限制

- Android 现有构建、单元测试和真机语音验收不能因桌面端改造而回退。
- macOS 端必须在真实 Mac 环境验证窗口、文件访问、音频输入、TTS 和识别流程；不能只依赖 JVM 测试。
- 语音识别适配器未完成前，macOS 不承诺自动跟读功能，只提供手动模式。
