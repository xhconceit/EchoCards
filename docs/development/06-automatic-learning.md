...
| 用例 | 操作与预期 | 状态 |
|---|---|---|
| A01 | 自动播放在 TTS 完成前保持正面；完成后翻面，停留后进入下一张。 | JVM 通过（`LearningEngineFollowAlongTest.a01`、`LearningEngineTest.autoPlayShowsMemoryTipBeforeEveryAdvance`）；真机未执行 |
| A02 | 自动跟读在朗读期间不收音；朗读完成后提示用户跟读并开启识别。 | JVM 通过（`a02`）；真机未执行 |
| A03 | 用户只读开头、漏读结尾或重复部分内容；不自动判定完成。完整读完才前进一张。 | JVM 通过（`a03_*`、`V1TextMatcherTest`、`CompletionTrackerTest`）；真机未执行 |
| A04 | 相同完成回调到达两次；只保存一条完成记录、只前进一张。 | JVM 通过（`a04`）；真机未执行 |
| A05 | 暂停、重读、切卡或切模式后收到旧 `operationId` 回调；当前状态不受影响。 | JVM 通过（`a05_*`）；真机未执行 |
| A06 | 跟读完成记录保存失败；不翻面、不前进，显示可重试错误。 | JVM 通过（`a06`）；真机未执行 |
| A07 | 后台、音频中断或退出；停止朗读和收音，返回前台保持暂停。 | JVM 通过（`a07_*`）；真机未执行（`StudySpeechTest.followAlongListensAfterSpeechThenStaysPausedInBackground` 待跑） |

真机用例需要连接目标设备（24122RKC7C）后执行 `./gradlew :app:connectedDebugAndroidTest`，
并手工走查一次三种模式；设备型号、Android 版本与结果记到本文件。
